// pages/mytravelcode/mytravelcode.js
...
Page({
  ...
  onSubmit: function() {
    ...
    if(!this.data.lock) {
      ...
      // 上传到云端数据库
      wx.cloud.uploadFile({
        cloudPath: `travelCodeImgs/${Math.floor(Math.random()*100)}_${Date.now()}.${this.data.tempImgList[0].match(/\.(\w+)$/)[1]}`,
